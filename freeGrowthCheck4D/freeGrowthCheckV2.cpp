		  ...
		  ) 
{
	...
	DistanceTransformF(D2, iL, DistanceTransformMode_CheckerBoardEuclid, ndimL, dimsL);

	vector<unsigned char> F(nvoxels, 0);
	vector<CParticle> vfront = collectSphericalFront(F, L, D2, vpclick, v, dimsL);
	...
	if(nlhs >= 5)
	{
		plhs[4] = StoreData(D2,mxSINGLE_CLASS,ndimL,dimsL);
	}
	mexUnlock();
#else
	return true;
