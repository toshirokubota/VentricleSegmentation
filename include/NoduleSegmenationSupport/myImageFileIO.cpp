#if !defined _myImageFileIO_cpp_
#define _myImageFileIO_cpp_
//#include "Stdafx.h"
...
template <class Item>
istream &operator>>( istream& input, myImage<Item>& image )
{
  Item item;
...
//
template <class Item>
ifstream& operator>>( ifstream& input, myImage<Item>& image )
{
  int bands, rows, cols;
...
//
template <class Item>
ofstream& operator<<(ofstream& output, const myImage<Item>& image)
{
...
template <class Item>
int
myImage<Item>:: ReadPnmFile(const char* filename) {
  FILE  *fp;
  size_t bytes_read;
...
template <class Item>
int
myImage<Item>:: WritePnmFile(const char* filename, pnm_type type, int norm) const {
...
template<class Item>
void
InteractiveImageWrite(const myImage<Item>& image, char* filename, int normalize) {
...
template <class Item>
int
myImage<Item>:: ReadRawFile(const char* filename) {
  int bands,rows,cols;
  printf("Reading data from %s\n", filename);
...
template <class Item>
int
myImage<Item>:: WriteRawFile(const char* filename) const {
