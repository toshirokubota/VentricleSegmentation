...
int
sanityCheck(vector<particle_cluster*>& clusters) {
	int numViolations = 0;
	for (int i = 0; i < clusters.size(); ++i)
	{
		particle_cluster* pc = clusters[i];
		for (set<particle_cluster*>::iterator it = pc->ascendents.begin(); it != pc->ascendents.end(); ++it)
		{
			particle_cluster* qc = *it;
			if (qc->descendents.find(pc) == qc->descendents.end())
			{
				numViolations++;
			}
		}
		for (set<particle_cluster*>::iterator it = pc->descendents.begin(); it != pc->descendents.end(); ++it)
		{
			particle_cluster* qc = *it;
			if (qc->ascendents.find(pc) == qc->ascendents.end())
			{
				numViolations++;
			}
		}
...
/*
pc has multiple descendents.
This function splits the cluster into multiple parts based on proximity so that
each gets a single descendent.
*/
vector<particle_cluster*> split(particle_cluster* pc)
{
	vector<particle_cluster*> vc;
	for (set<particle_cluster*>::iterator it = pc->descendents.begin(); it != pc->descendents.end(); ++it)
	{
		vc.push_back(*it);
	}
	//for each particle, assing the closest decendent.
	vector<int> vidx(pc->particles.size(), -1);
	for (int i = 0; i < pc->particles.size(); ++i)
	{
		CoreParticle* p = pc->particles[i];
		float mind = std::numeric_limits<float>::infinity();
		for (int j = 0; j < vc.size(); ++j)
		{
			particle_cluster* qc = vc[j];
			for (int k = 0; k < qc->particles.size(); ++k)
			{
				CoreParticle* q = qc->particles[k];
				float dx = p->x - q->x;
				float dy = p->y - q->y;
				float dz = p->z - q->z;
				float dt = p->t - q->t;
				float dval = sqrt(dx*dx + dy*dy + dz*dz + dt*dt);
				if (dval < mind)
				{
					mind = dval;
					vidx[i] = j;
	...
	//group particles based on the closest cluster.
	vector<vector<CoreParticle*>> groups(vc.size());
	for (int i = 0; i < pc->particles.size(); ++i)
	{
		CoreParticle* p = pc->particles[i];
		int j = vidx[i];
	...
	//make new clusters
	CoreClusterFactory& factory = CoreClusterFactory::getInstance();
	vector<particle_cluster*> newcl;
	for (int i = 0; i < groups.size(); ++i)
	{
		particle_cluster* qc = factory.makeCoreCluster(groups[i], pc->gen);
		qc->descendents.insert(vc[i]);
		vc[i]->ascendents.insert(qc);
		qc->ascendents = pc->ascendents;
		for (set<particle_cluster*>::iterator it = pc->ascendents.begin(); it != pc->ascendents.end(); ++it)
		{
			particle_cluster* rc = *it;
			rc->descendents.insert(qc);
		}
		newcl.push_back(qc);
	}
	//update existing ascendents/descendents.
	for (int i = 0; i < vc.size(); ++i)
	{
		vc[i]->ascendents.erase(find(vc[i]->ascendents.begin(), vc[i]->ascendents.end(), pc));
	}
	for (set<particle_cluster*>::iterator it = pc->ascendents.begin(); it != pc->ascendents.end(); ++it)
	{
		particle_cluster* qc = *it;
		qc->descendents.erase(find(qc->descendents.begin(), qc->descendents.end(), pc));
	}
	pc->ascendents.clear();
	pc->descendents.clear();

	//update cluster link for core particles.
	for (int i = 0; i < newcl.size(); ++i)
	{
		particle_cluster* qc = newcl[i];
		for (int j = 0; j < qc->particles.size(); ++j)
		{
			CoreParticle* p = qc->particles[j];
			p->cluster = qc;
		}
	}
	return newcl;
...
vector<particle_cluster*>
clusterSplitting(vector<particle_cluster*>& clusters)
{
	set<particle_cluster*> Q;
	for (int i = 0; i < clusters.size(); ++i)
	{
		if (clusters[i]->descendents.empty())
		{
			Q.insert(clusters[i]);
		}
	}
	vector<particle_cluster*> newset;
	while (Q.empty() == false)
	{
		...
		{
			particle_cluster* pc = *it;
			Q2.insert(pc->ascendents.begin(), pc->ascendents.end());
			if (pc->descendents.size() >= 2)
			{
				vector<particle_cluster*> vc = split(pc);
				newset.insert(newset.end(), vc.begin(), vc.end());
			}
			else
			{
				newset.push_back(pc);
	...
	printf("# clusters changed from %d to %d.\n", clusters.size(), newset.size());
	return newset;
}

float 
bondingStrength(particle_cluster* a, particle_cluster* b, 
				vector<vector<int>>& nbh, 
				vector<CoreParticle*>& pm, int ndim, const int* dims)
{
	int count = 0;
	set<CoreParticle*> pset;
	for (int i = 0; i < a->particles.size(); ++i)
	{
		vector<int> sub = coreParticle2Index(a->particles[i], ndim);
		...
		for (int n = 0; n < nbh.size(); ++n)
		{
			...
			{
				int idx = Sub2Ind(sub, nbh[n], ndim, dims);
				if (pm[idx] != NULL && pm[idx]->cluster == b)
				{
					pset.insert(pm[idx]);
					bneighbor = true;
				}
	...
	//return (float)(count + pset.size()) / (a->size() + b->size());
	return (float)(count) / (a->size());
}

void
colorParticles(vector<particle_cluster*>& clusters, vector<particle_cluster*>& cores, int ndim, const int* dims)
{
	vector<CoreParticle	*> pm(numberOfElements(ndim, dims), (CoreParticle*)NULL);
	for (int i = 0; i < clusters.size(); ++i)
	{
		particle_cluster* pc = clusters[i];
		for (int j = 0; j < pc->size(); ++j)
		{
			CoreParticle* p = pc->particles[j];
			SetVoxel(pm, p, p, ndim, dims);
		}
	}

	map<particle_cluster*, particle_cluster*> cmap;
	for (int i = 0; i < clusters.size(); ++i)
	{
		particle_cluster* pc = clusters[i];
		pc->value = 0; //used to store the bonding strength
		pc->label = 0;
		cmap[pc] = NULL;
	}
	set<particle_cluster*> Q;
	...
	{
		cores[i]->value = 1.0f; //core has the maximum bonding to begin with, so that its ownership won't change
		cmap[cores[i]] = cores[i];
		Q.insert(cores[i]);
		cores[i]->label = (i+1);
	}
	...
	while (Q.empty() == false)
	{
		...
		{
			particle_cluster* pc = *it;
			//if (pc->id == 5754) continue; //TK - skip this one for debuggin..

			for (set<particle_cluster*>::iterator jt = pc->ascendents.begin(); jt != pc->ascendents.end(); ++jt)
			{
				particle_cluster* qc = *jt;
				float bval = bondingStrength(pc, qc, nbh, pm, ndim, dims);
				if (bval > qc->value)
				{
					qc->value = bval;
					cmap[qc] = pc; // cmap[pc];
	...
	{
		particle_cluster* pc = clusters[i];
		while (pc != NULL && pc != cmap[pc])
		{
			pc = cmap[pc];
		}
		for (int j = 0; j < clusters[i]->particles.size(); ++j)
		{
			clusters[i]->particles[j]->label = pc == NULL ? -1: pc->label;
		}
	}
}
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	//int numViolations = sanityCheck(clusters);
	//clusters = clusterSplitting(clusters);
	vector<particle_cluster*> core = pickCore(clusters, costCutoff);
	colorParticles(clusters, core, ndimL, dimsL);
	...
	if (nlhs >= 5)
	{
		vector<int> T2(nvoxels, 0);
		for (int i = 0; i < clusters.size(); ++i)
		{
			for (int j = 0; j < clusters[i]->particles.size(); ++j)
			{
				CoreParticle* p = clusters[i]->particles[j];
				SetVoxel(T2, p, clusters[i]->id, ndimL, dimsL);
			}
		}
		plhs[4] = StoreData(T2, mxINT32_CLASS, ndimL, dimsL);
	}
	mexUnlock();
	factory.clean();
