...
colorParticles(vector<particle_cluster*>& clusters, vector<particle_cluster*>& cores, int ndim, const int* dims)
{
	...
	vector<int> lm(numberOfElements(ndim, dims), 0);
	for (int i = 0; i < clusters.size(); ++i)
	{
	...
	vector<set<CoreParticle*>> vset(maxgen + 1);
	map<int, int> regionSize;
	for (int i = 0; i < cores.size(); ++i)
	{
		particle_cluster* pc = cores[i];
		regionSize[i+1] = 0;
		for (int j = 0; j < pc->size(); ++j)
		{
			CoreParticle* p = pc->particles[j];
			SetVoxel(lm, p, p->gen, ndim, dims);
			p->label = i+1;
			vset[p->gen].insert(p);
		}
	}
	...
	vector<vector<int>>& nbh = NeighborhoodFactory::getInstance(ndim).neighbor4; //MakeEightNeighborhood(ndim, dims);
	set<CoreParticle*> Q = vset[maxgen];
	int g = 1;
	while (Q.empty() == false)
	{
		set<CoreParticle*> Q2;
		...
		for (set<CoreParticle*>::iterator it = Q.begin(); it != Q.end(); ++it)
		{
			...
			for (int n = 0; n < nbh.size(); ++n)
			{
				if (NeighborCheck(sub.begin(), nbh[n].begin(), ndim, dims))
				{
					...
					if (pm[idx] != NULL)
					{
						if (lm[idx] == 0) // || rand() % 2 == 0)
						{
							if (candidates.find(idx) == candidates.end())
							{
								candidates[idx] = set<int>();
							}
							candidates[idx].insert(p->label);
						}
					}
				}
			}
		}
		for (map<int,set<int>>::iterator it = candidates.begin(); it != candidates.end(); ++it)
		{
			set<int> sc = it->second;
			int maxregion = 0;
			int id = -1;
			for (set<int>::iterator jt = sc.begin(); jt != sc.end(); ++jt)
			{
				if (*jt > maxregion)
				{
					maxregion = regionSize[*jt];
					id = *jt;
				}
			}
			CoreParticle* q = pm[it->first];
			q->label = id;
			lm[it->first] = g;
			Q2.insert(q);
			regionSize[id] = regionSize[id] + 1;
		}
		Q = Q2;
		g++;
	}
