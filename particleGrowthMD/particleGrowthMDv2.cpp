...
pickCore(vector<particle_cluster*>& pclusters)
{
	vector<particle_cluster*> core;
	for (int i = 0; i < pclusters.size(); ++i)
	{
		if (pclusters[i]->descendents.empty())
		{
			core.push_back(pclusters[i]);
		}
		else
		{
			//check if its ancestors are in disjoint components
			set<particle_cluster*> S;
			S.insert(pclusters[i]);
			int gen = pclusters[i]->gen;
			int np = pclusters[i]->particles.size();
			for (int k = 0; k < 1; ++k)
			{
				set<particle_cluster*> S2;
				for (set<particle_cluster*>::iterator it = S.begin(); it != S.end(); ++it)
				{
					S2.insert((*it)->ascendents.begin(), (*it)->ascendents.end());
				}

				int nc = numDisjointClusters(S2);
				if (nc > 1)
				{
					core.push_back(pclusters[i]);
					break;
	...
	printf("There are %d core clusters.\n", core.size());
	return core;
}
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	vector<particle_cluster*> core = pickCore(clusters);
