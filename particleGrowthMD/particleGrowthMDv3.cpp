...
//two clusters have strong bond if w/d is large and len is small.
float measure(float w, float len, float d)
{
	return w/d / len;
}

void
colorParticles(vector<CoreParticle*>& particles, int ndim, const int* dims)
{
	vector<CoreParticle*> core;
	for (int i = 0; i < particles.size(); ++i)
	{
	...
	SymmetricalMap<float> dpmap(groups.size(), 0.0f); //distanace between two clusters.
	...
	while (Q.empty() == false)
	{
		...
		for (int i = 0; i < Q.size(); ++i)
		{
			...
			for (int n = 0; n < p->neighbors.size(); ++n)
			{
				...
				{
					/*if (dpmap.valid(p->label, q->label))
					{
						wpmap.set(p->label, q->label, Max(p->dval, q->dval));
						dpmap.set(p->label, q->label, sqrt((p->value + 1) * (q->value + 1)));
					}
					else*/
					{
						if (wpmap.get(p->label, q->label) < Max(p->dval, q->dval))
						{
							wpmap.set(p->label, q->label, Max(p->dval, q->dval));
							dpmap.set(p->label, q->label, sqrt((p->value + 1) * (q->value + 1)));
							if (wpmap._index(p->label, q->label) == wpmap._index(5, 10))
							{
								printf("(%d, %d): w=%f, len=%f, d=%f, d=%f, measure=%f\n",
									p->label + 1, q->label + 1, 
									wpmap.get(p->label, q->label), dpmap.get(p->label, q->label), p->dval, q->dval,
									measure(wpmap.get(p->label, q->label), dpmap.get(p->label, q->label), Min(p->dval, q->dval)));
							}
						}
					}
				}
	...
	for (int i = 0; i < groups.size(); ++i)
	{
		for (int j = i + 1; j < groups.size(); ++j)
		{
			...
			{
				printf("(%d, %d): w=%f, len=%f, d=%f, d=%f, measure=%f\n",
					i + 1, j + 1, wpmap.get(i, j), dpmap.get(i, j), groups[i][0]->dval, groups[j][0]->dval,
					measure(wpmap.get(i, j), dpmap.get(i, j), Min(groups[i][0]->dval, groups[j][0]->dval)));
			}
		}
	}
}
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	colorParticles(particles, ndimL, dimsL);
