...
/*CParticle4D
particleNdim(vector<int>& loc, int ndim)
{
	switch (ndim)
	{
	case 1:
		return CParticle4D(loc[0]);
	case 2:
		return CParticle4D(loc[0], loc[1]);
	case 3:
		return CParticle4D(loc[0], loc[1], loc[2]);
	case 4:
		return CParticle4D(loc[0], loc[1], loc[2], loc[3]);
	default:
...
template<class T>
bool
onSurface(const vector<T>& A, vector<int> sub, vector<vector<int>>& nbh, int ndim, const int* dims) 
{
	if (A[p])
	{
			  ...
			  const int* dims)
{
	...
	vector<vector<int>> vnbh;
	for (int i = 0; i < nbh.size(); ++i)
	{
		vnbh.push_back(Ind2SubCentered(nbh[i], ndim, dims));
	}
	for (int i = 0; i < nvoxels; ++i)
	{
		if (L[i])
		{
			...
			if (onSurface(L, sub, vnbh, ndim, dims))
			{
			...
			const int* dims)
{
	...
	while (core.empty() == false)
	{
		...
		for (int i = 0; i < core.size(); ++i)
		{
			...
			{
				if (NeighborCheck(loc, nbh[n], ndim, dims))
				{
					CoreParticle* q = M[loc + nbh[n]];
					...
					const int* dims)
{
	...
	for (int g = 0; g < group.size(); ++g)
	{
		...
		{
			if (assignExit(group[g], M, ndim, dims) == true) //there is an exit particle
			{
					...
					const int* dims)
{
	...
	while (Q.empty() == false)
	{
		...
		for (int i = 0; i<Q.size(); ++i)
		{
			...
			for (int n = 0; n < nbh.size(); ++n)
			{
				if (NeighborCheck(loc, nbh[n], ndim, dims))
				{
					...
					if (L[idx] && S[idx] == 0)
					{
						CoreParticle* p = mp[loc + nbh[n]];
						if (p == NULL)
						{
							p = coreParticleNdim(Ind2Sub(idx, ndim, dims), ndim);
							p->gen = gen;
							mp[idx] = p;
							Q2.push_back(p);
		...
		core = removeIsolatedCore(core, mincore, mp, ndim, dims);
...
*/
bool
erode(vector<unsigned char>& L, const vector<int>& mask,
		int ndim,
		const int* dims)
{
		...
		for (int k = 0; k < dims[2]; ++k)
		{
			for (int i = 0; i < dims[1]; ++i)
			{
				for (int j = 0; j < dims[0]; ++j)
				{
					...
					{
						if (
							GetData3(L, j - 1, i, k, dims[0], dims[1], dims[2], (unsigned char)0) == 0 ||
							GetData3(L, j + 1, i, k, dims[0], dims[1], dims[2], (unsigned char)0) == 0 ||
							GetData3(L, j, i - 1, k, dims[0], dims[1], dims[2], (unsigned char)0) == 0 ||
							GetData3(L, j, i + 1, k, dims[0], dims[1], dims[2], (unsigned char)0) == 0 ||
							GetData3(L, j, i, k - 1, dims[0], dims[1], dims[2], (unsigned char)0) == 0 ||
							GetData3(L, j, i, k + 1, dims[0], dims[1], dims[2], (unsigned char)0) == 0
							)
						{
			...
			for (int k = 0; k < dims[2]; ++k)
			{
				for (int i = 0; i < dims[1]; ++i)
				{
					for (int j = 0; j < dims[0]; ++j)
					{
						...
						{
							if (
								GetData4(L, j - 1, i, k, m, dims[0], dims[1], dims[2], dims[3], (unsigned char)0) == 0 ||
								GetData4(L, j + 1, i, k, m, dims[0], dims[1], dims[2], dims[3], (unsigned char)0) == 0 ||
								GetData4(L, j, i - 1, k, m, dims[0], dims[1], dims[2], dims[3], (unsigned char)0) == 0 ||
								GetData4(L, j, i + 1, k, m, dims[0], dims[1], dims[2], dims[3], (unsigned char)0) == 0 ||
								GetData4(L, j, i, k - 1, m, dims[0], dims[1], dims[2], dims[3], (unsigned char)0) == 0 ||
								GetData4(L, j, i, k + 1, m, dims[0], dims[1], dims[2], dims[3], (unsigned char)0) == 0 ||
								GetData4(L, j, i, k, m - 1, dims[0], dims[1], dims[2], dims[3], (unsigned char)0) == 0 ||
								GetData4(L, j, i, k, m + 1, dims[0], dims[1], dims[2], dims[3], (unsigned char)0) == 0
								)
							{
						...
						const int* dims)
{
	...
	while (Q.empty() == false)
	{
		...
		for (int i = 0; i < Q.size(); ++i)
		{
			...
			int loc = Sub2Ind(coreParticle2Index(p, ndim), ndim, dims);
			for (int n = 0; n < nbh.size(); ++n)
			{
				if (NeighborCheck(loc, nbh[n], ndim, dims))
				{
...
{   
	...
	vector<core_cluster*> coregroup = groupCoreParticles(particles, init, nclusters, ndimL, dimsL);
