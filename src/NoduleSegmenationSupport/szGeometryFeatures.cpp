						...
						const int* dims) 
{
	...
	vector<CFeature> features = geometryFeatures();
...
geometryFeaturesNew()
{
	int numFeatures = 4;
	vector<CFeature> features(numFeatures);
	features[0].Name = "DistanceApartRatio";
	features[1].Name = "MaxSegSphericity";
	features[2].Name = "SegSphericityRatio";
	features[3].Name = "DmapDisparity";
	return features;
}
...
{
	int ndim = 3;
	vector<CFeature> features = geometryFeaturesNew();
	int nvoxels = numberOfElements(ndim, dims);
	...
	{   
		...
		features[1].Value = maxSp;
		if(maxSp>0)
			features[2].Value = 1 - maxSp0/maxSp;
	}
	{   
...
erosionFeatures()
{
	int numFeatures = 4;
	vector<CFeature> features(numFeatures);
	features[0].Name = "JointBoundaryFitness1";
	features[1].Name = "JointBoundaryFitness2";
	features[2].Name = "JointBoundaryFitness3";
	features[3].Name = "JointBoundaryFitness4";
	return features;
}
...
computeErosionFeatures(const vector<unsigned char>& B, //segmentation
					   ...
					   const int* dims)
{
	vector<CFeature> features = erosionFeatures();
	...
	if(dmaxB>0)
	{
		features[0].Value = exp(-dmaxJ/(dmaxB*dmaxB));
		features[1].Value = 1.0/(1.0 + exp(dmaxJ-dmaxB));
		features[2].Value = 1.0/(1.0 + exp(dmaxJ*dmaxJ-dmaxB*dmaxB));
		if(ns)
			features[3].Value = 1.0 - (double)nj/(double)ns;
	}
	return features;
}
